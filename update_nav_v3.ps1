...
$headTemplate = @'
    ...
    <meta name="description" content="L'Organisation MDH International Togo - PAGE_DESCRIPTION">
...
foreach ($page in $pages) {
    ...
    $pageHead = $headTemplate.Replace("PAGE_TITLE", $page.title).Replace("PAGE_DESCRIPTION", $page.description)
    
    # Préparer la navigation avec des data-page corrects
    $filePage = [System.IO.Path]::GetFileNameWithoutExtension($page.file)
    $pageNav = $navigationTemplate -replace '(data-page=")index"', "`${1}$filePage`""
    
    # Lire le contenu existant du fichier
    $content = [System.IO.File]::ReadAllText($page.file)
    
    # Remplacer la section head
    $content = [System.Text.RegularExpressions.Regex]::Replace($content, '(?s)(<head>).*(</head>)', "`${1}`n$pageHead`n</head>")
    
    # Remplacer le header
    $content = [System.Text.RegularExpressions.Regex]::Replace($content, '(?s)(<!-- Header -->).*?(</header>)', $pageNav)
    
    # Sauvegarder les modifications
    [System.IO.File]::WriteAllText($page.file, $content, [System.Text.Encoding]::UTF8)
    Write-Host "  - Succès"
}
